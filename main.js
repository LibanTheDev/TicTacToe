var Board = function () {
	...
	//@param we draw the X or O corresponding to the player or cpu turn
		drawToken = function (sign) {
			var compSign ;
			console.log(sign);
			if(sign === "X") compSign = "O";
			if(sign === "O") compSign = "X";
			for (var i = 0; i < 9; i++) {
				...
				if(this.board[i] === 0) {
					...
					if (this.board[i] === 1){
						this.cells[i].innerHTML = sign;
					} else {
						this.cells[i].innerHTML = compSign;
					}
				}
			}
		};
	...
	//when starts a new game makes start either the cpu or    //player. this is for even games starts the cpu.
		reset = function() {
			...
			if(!isEven(this.turn)) {
				...
				this.board.drawToken(this.sign);

		},
 	//when a square is clicked this event is launch
		startMove = function(num, sign) {
...
$(document).ready(function () {
	...
	$("#button1").click(function(){
		game.sign = this.innerHTML;
		flag = true;
		$token.text("You have selected X");
		$(this).toggleClass("selected");
		if($('#button2').hasClass('selected')){
			$('#button2').toggleClass('selected');
		}
		/*$("#button2").toggleClass("selected");*/
	});
	$("#button2").click(function(){
		game.sign = this.innerHTML;
		flag = true;
		$token.text("You have selected O");
		$(this).toggleClass("selected");
		if($('#button1').hasClass('selected')){
			$('#button1').toggleClass('selected');
		}
	...
	$(".item").click(function () {
		if(!flag || flag === null) {
			console.log(flag);
			$token.text("Please select an input");
		}else if (flag){
		...
		game.startMove(parseInt(this.getAttribute("num")), game.sign);
		}
	});
